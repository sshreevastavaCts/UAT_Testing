...
export default class UserAttemptResult extends NavigationMixin(LightningElement) {
    // ...existing code...
    ...
    saveResultsToApex() {
        ...
        const  percentage = (isCorrectCount / this.questions.length) * 100;
        ...
        saveResults({ quizAnswers: this.responses, QR_Answers: this.QR_responses })
            .then(resultId => {
                console.log('Responses saved successfully! Quiz_Result__c Id:', resultId);
                // Redirect to the Quiz_Result__c record page
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Success',
            ...
            .catch(error => {
                this.dispatchEvent(
                    new ShowToastEvent({
                        title: 'Error',
                        message: 'Error saving quiz results: ' + (error.body && error.body.message ? error.body.message : error.message),
                        variant: 'error'
                    })
                );
