...
export default class UserAttemptResult extends LightningElement {
    ...
    saveResultsToApex() {
        ...
        const convertedDate = parseISTDate(new Date().toLocaleString());
    ...
    parseISTDate(dateString) {
        const [datePart, timePart] = dateString.split(', ');
        const [day, month, year] = datePart.split('/').map(Number);
        const [hours, minutes, seconds] = timePart.split(':').map(Number);

        // Create the date in IST (UTC+5:30)
        const istDate = new Date(Date.UTC(year, month - 1, day, hours - 5, minutes - 30, seconds));
        return istDate.toISOString(); // This returns date in UTC ISO string
    }
}